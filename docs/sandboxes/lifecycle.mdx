...
The relay retains the blocked message and retries with a delay capped at
100 milliseconds. Its bounded input channel applies backpressure to existing
clients, so they cannot accumulate unlimited queued input in the relay. When
