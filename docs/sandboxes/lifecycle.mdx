...
## Guest agent input stalls

If the guest agent's input queue remains full for 60 seconds while a message
is waiting, the relay rejects new connection attempts. Existing connections,
pending requests, guest output, and log capture remain active. The VM and its
workloads are not terminated because of the stall.

The relay retains the blocked message and waits for console capacity notifications
(on Windows, it retries at bounded intervals). Its bounded input channels apply
backpressure to existing clients, so they cannot accumulate unlimited queued
input in the relay. This applies to both control and bulk input lanes. When
the guest consumes input again, queued messages resume in order and the relay
accepts new connections again. Request timeouts remain the caller's choice;
a timeout does not prove that a guest operation did not run.
