...
Use disk snapshots for installed dependencies, prepared workspaces, and saved files. Stop the source first to include writes buffered in memory. Local live capture is also supported but is crash-consistent; cloud requires a stopped or crashed source.
...
Buffered guest writes may be missing, and filesystem recovery may run. Full capture does not automatically sync guest filesystems. A tmpfs-root snapshot cannot restore disk-only.
...
Supply a host binding explicitly. These local restore options work for disk and full snapshots:
...
Relaxed mode does not grant host access, recreate missing files, or bypass integrity checks. Unmapped host filesystems remain inaccessible; omitting them does not fail strict validation.
