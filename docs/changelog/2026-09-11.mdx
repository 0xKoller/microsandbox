...
- HTTP host policy checks now match the hostname that policy allowed, closing a gap where a mismatched `Host` header could bypass a hostname-based allow rule.
