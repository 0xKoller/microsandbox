...
Host resources are not inherited by default. Explicitly map volumes, ports, and vsock endpoints. Unmapped filesystems can return I/O errors, while cached bytes may remain readable; warnings appear even with `--quiet`. The root disk is always restored. See [mount behavior](/sandboxes/snapshots#mount-behavior).
...
| Flag | Description |
| --- | --- |
| `--name NAME` | One child name; conflicts with `--names` |
